...
static NSString* _accessToken = @"Z";
...
+(void) requireAppCheckToken{
    ...
    [appCheck tokenForcingRefresh:true completion:^(FIRAppCheckToken* _Nullable token, NSError* _Nullable error){
        if(token){
            NSLog(@"AppCheck Success: %@", token.token);
            _accessToken = token.token;
        }
        else{
            NSLog(@"AppCheck Error: %@", error.localizedDescription);
        }
    }];
